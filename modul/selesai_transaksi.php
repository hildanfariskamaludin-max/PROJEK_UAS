...
if (isset($_GET['id'])) {
    ...
    $query_cari = mysqli_query($koneksi, "SELECT id_unit_ps FROM transaksi WHERE id = '$id_transaksi'");
    ...
    $query_update_transaksi = "UPDATE transaksi SET status_transaksi = 'selesai', jam_selesai = '$jam_sekarang' WHERE id = '$id_transaksi'";
    
    if (mysqli_query($koneksi, $query_update_transaksi)) {
        
        // 3. Ubah kembali status unit PS menjadi 'ready' agar bisa disewa lagi
        mysqli_query($koneksi, "UPDATE unit_ps SET status = 'ready' WHERE id = '$id_unit_ps'");
