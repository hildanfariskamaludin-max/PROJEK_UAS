...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 2. Generate Otomatis Kode Transaksi (Contoh: TRX-20260614-001)
    $tanggal_sekarang = date('Ymd');
    $kode_transaksi   = "TRX-" . $tanggal_sekarang . "-" . rand(100, 999);
    ...
    // 4. Hitung Total Bayar (Ambil harga per jam dari tabel unit_ps)
    $query_ps  = mysqli_query($koneksi, "SELECT harga_per_jam FROM unit_ps WHERE id = '$id_unit_ps'");
    ...
    $nik_dummy = "DUMMY-" . rand(1000, 9999);
    mysqli_query($koneksi, "INSERT INTO pelanggan (nik_ktp, nama_pelanggan, no_hp) VALUES ('$nik_dummy', '$nama_pelanggan', '-')");
    $id_pelanggan_baru = mysqli_insert_id($koneksi);

    // Insert ke tabel utama transaksi
    $query_transaksi = "INSERT INTO transaksi (kode_transaksi, id_pelanggan, id_unit_ps, jam_mulai, jam_selesai, total_bayar, ttd_digital, status_transaksi) 
    ...
    if (mysqli_query($koneksi, $query_transaksi)) {
        ...
        mysqli_query($koneksi, "UPDATE unit_ps SET status = 'disewa' WHERE id = '$id_unit_ps'");
